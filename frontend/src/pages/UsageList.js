...
import { usageApi, chemicalApi, labApi, userApi } from '../services/api';
...
function UsageList() {
  ...
  const loadData = async () => {
    try {
      const [usagesRes, chemicalsRes, labsRes, usersRes] = await Promise.all([
        usageApi.getAll(),
        chemicalApi.getAll(),
        labApi.getAll(),
        userApi.getAll(),
  ...
  return (
    <div>
      ...
      <div className="card">
        <table>
          ...
          <tbody>
            {usages.map((usage) => (
              <tr key={usage.id}>
                <td>{getChemicalName(usage.chemicalId)}</td>
                <td>{usage.quantity}</td>
                <td>{getUserName(usage.userId)}</td>
                <td>{getLabName(usage.labId)}</td>
                <td>{usage.experimentProject || '-'}</td>
                <td>{usage.requestTime}</td>
                <td>{getStatusBadge(usage.approvalStatus)}</td>
                <td>{getUserName(usage.firstApproverId)}</td>
                <td>{getUserName(usage.secondApproverId)}</td>
                <td>{usage.wasteQuantity || '-'}</td>
                <td>
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreateSubmit}>
              ...
              <div className="form-group">
                ...
                <input
                  type="text"
                  value={formData.experimentProject}
                  onChange={(e) => setFormData({ ...formData, experimentProject: e.target.value })}
                />
              </div>
              <div className="form-group">
                <label>备注</label>
      ...
      {showApproveModal && selectedUsage && (
        <div className="modal-overlay" onClick={() => setShowApproveModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <h3>审批领用申请</h3>
            <div style={{ marginBottom: '1rem' }}>
              <p><strong>危化品:</strong> {getChemicalName(selectedUsage.chemicalId)}</p>
              <p><strong>数量:</strong> {selectedUsage.quantity}</p>
              <p><strong>领用人:</strong> {getUserName(selectedUsage.userId)}</p>
              <p><strong>当前状态:</strong> {getStatusBadge(selectedUsage.approvalStatus)}</p>
